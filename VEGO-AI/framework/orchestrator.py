...
async def _answer_lang_questions(
    ...
    prompt = a1.answer_language_question_prompt(
        language_name=state.language_template.get("language_name", ""),
...
async def _answer_dom_questions(
    ...
    prompt = a2.answer_domain_question_prompt(
        domain_description=state.reference_guidelines.get("_domain_description", ""),
        reference_guidelines=state.reference_guidelines,
        questions=questions,
        domain_identifier=state.reference_guidelines.get("domain_identifier", ""),
    )
...
async def phase2_build_reference_guidelines(
    ...
    for round_n in range(1, MAX_QA_ROUNDS + 1):
        ...
        # Stash domain_description for agent2 Q&A answer calls later
        result["_domain_description"] = domain_description
        current_guidelines = result

        q_lang = result.get("questions_to_language_advisor", [])
        q_dom = result.get("questions_to_domain_advisor", [])
    ...
    state.save(state_path)
    logger.info("Phase 2 complete. %d guideline(s) produced.",
                len(current_guidelines.get("reference_guidelines", [])))  # type: ignore[union-attr]
...
async def _phase3_one_case(
    ...
    async with sem:
        ...
        for round_n in range(1, MAX_QA_ROUNDS + 1):
            ...
            q_lang = audit_result.get("questions_to_language_advisor", [])
            q_dom = audit_result.get("questions_to_domain_advisor", [])
...
def _merge_resolved_into_cv(cv: dict, resolved: dict) -> dict:
    ...
    merged_mapping = []
    for entry in cv.get("existing_mapping", []):
