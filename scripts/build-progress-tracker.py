...
def main(argv=None):
    ...
    if args.run_tests:
        ...
        tests_str = "94 passed _(rerun with --run-tests to refresh)_"

    # ---- build region bodies --------------------------------------------------
    stamp = (f"As of {now} · branch `{branch}` · HEAD `{head}` · evidence baseline commit "
             f"`{dash_commit}` · generated by `scripts/build-progress-tracker.py`")
