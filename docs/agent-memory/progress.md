...
| --- | --- | --- | --- | --- |
| TASK-001 | 2026-06-11 | Partially done | Durable revert support started by adding `.gitignore` and initializing Git. | Complete `TASK-002` baseline commit. |
| TASK-002 | 2026-06-11 | Open | Create first Git baseline commit. | Review `git status`, configure Git identity, then commit selected files. |
...
1. Review `git status` and create a baseline commit.
2. Audit data/IRB sensitivity before publishing or sharing.
3. Convert existing package results into experiment cards.
4. Add tests around evaluator/scoring/parsing behavior.
5. Continue running the prompt start/end memory scripts for every meaningful prompt.
