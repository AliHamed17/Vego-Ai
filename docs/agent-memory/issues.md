...
| --- | --- | --- | --- | --- | --- | --- |
| ISS-009 | 2026-06-14 | Codex | High | In review | The visualizer could silently show stale or mismatched model/result pairs because aggregate selection used loose substring matching and did not clear the previous model when no match existed. PR #7 fixes this with exact `<case_id>_` matching, stale-model clearing, a persistent pairing banner, and helper regression tests. | Review and merge PR #7, then verify the GUI on a real display with representative Cheers/ParkWise UCD/CD cases. |
