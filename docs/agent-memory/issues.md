...
| ISS-006 | 2026-06-12 | Codex | Medium | Open | M4B-1 memory-informed parallel comparison has an initial EXP-001 mechanism/readiness run and an EXP-002 labeling package, but no completed generalization-safe expert-label evaluation yet. | Fill the EXP-002 labeling sheet, rerun EXP-001 or the next evaluation pass, and do not claim reusable memory improves AI variability interpretation until evidence exists. |
