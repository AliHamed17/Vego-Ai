...
| ISS-006 | 2026-06-12 | Codex | Medium | Open | M4B-1 memory-informed parallel comparison has an initial EXP-001 mechanism/readiness run, but no generalization-safe expert-label evaluation yet. | Collect held-out/cross-setting expert labels, rerun EXP-001, and do not claim reusable memory improves AI variability interpretation until evidence exists. |
| ISS-007 | 2026-06-14 | Codex | Medium | Open | M4B/C4B can suffer evaluation leakage if memory from the same pattern is reused for that pattern; the first EXP-001 run has 3 same-pattern expert-labeled rows and 0 generalization-safe expert-labeled rows. | Keep same-pattern rows as mechanism validation only; require leave-one-pattern-out, cross-setting, cross-domain, cross-diagram, or expert-only holdout evaluation before making generalization claims. |
