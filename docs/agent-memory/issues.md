...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ISS-027 | 2026-07-30 | Codex (presentation audit) | High | High | Document production + human rehearsal | Open | No current July-29/August supervisor PPTX/PDF, presenter notes, rendered QA, or live rehearsal exists; candidacy presentation rules are also unverified. Legacy July decks contain superseded scope/claims and cannot be reused unchanged. | Confirm logistics, build from the current 12-checkpoint checklist, add source notes, render/inspect, run a dated human rehearsal, and archive the reviewed exact package. |
...
| ISS-021 | 2026-07-28 | Claude (deck fact-check) | Medium | High | Doc edit | Open | Tracked docs describe the corpus as "179 student models" (`docs/PROGRESS_TRACKER.md`, `scripts/build_thesis_evidence_package.py`), but 179 is the number of scored agentC ranking rows and includes 14 duplicate case_ids. Defensible counts: 83 distinct student model IDs (46 Cheers + 37 ParkWise), 165 distinct model x setting evaluations, 179 scored rows. `docs/research/baseline-characterization.md` correctly labels it "cases". | Standardise on "179 scored evaluations across 4 settings (83 distinct student models)" wherever the figure appears, before it reaches a thesis chapter. |
| ISS-020 | 2026-07-28 | Claude (deck fact-check) | Medium | Medium | Doc + rerun | Open | `EXPERIMENT_BENCHMARK_ANALYTICS_REPORT.md` states EXP-036 "meets the declared unified and parity overhead limits" and grades it MEASURED_PASS, while the pinned artifact `reports/generated/bigui_architecture/summary.json` records `engineeringTargetMet = false` (unifiedP95 check false at all three scales). The stored p95 confidence intervals also do not bracket their own point estimates, and the ratio moves between runs on the same machine (10X: 0.946-1.164). | Correct the analytics prose to "target NOT met", fix the interval computation, and repeat EXP-036 on a second machine before any latency figure enters the thesis. |
